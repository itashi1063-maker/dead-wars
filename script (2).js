...
class Character {
    ...
    draw() {
        ctx.save();
        // جعل البوت يظهر بلون محمر لتمييزه
        if (!this.isPlayer) {
            ctx.filter = "sepia(1) saturate(4) hue-rotate(-50deg)";
        }

        let dw = this.w, dh = this.h, dy = this.y;
        // تكبير حجم صورة الكومبو عند تفعيلها
        if (this.img === images.combo) {
            dw *= 1.8; dh *= 1.8; dy -= 100;
        }

        // عكس اتجاه الصورة بناءً على حركة اللاعب
        if (this.facing === -1) {
            ctx.translate(this.x + dw, dy); 
            ctx.scale(-1, 1);
    ...
    attack() {
        ...
        if (this.combo >= 3) {
            ...
            // تتابع صور الهجوم (إصلاح مشكلة اختفاء الصورة الثانية)
        ...
        // نظام الـ Cooldown لمدة 2 ثانية
        setTimeout(() => {
            this.canAtk = true;
